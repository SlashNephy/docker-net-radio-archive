...
RUN mkdir /myapp
WORKDIR /myapp
ARG REPOSITORY=https://github.com/yayugu/net-radio-archive
ARG BRANCH=master
...
CMD bundle exec rake db:create db:migrate && bundle exec whenever --update-crontab && chmod 0600 /var/spool/cron/crontabs/root && rsyslogd && /usr/sbin/cron -f
